...
mod tests {
    ...
    fn test_apply_migrations_records_latest_version() {
        ...
        // highest registered version. v017/v018 (reports strip), v019 (exam
        // strip), and v020 (entitlements strip) were all removed — the
        // registered migrations are now a contiguous v001..v016 chain.
        let conn = fresh_conn();
        ...
        let version: i32 = conn
            ...
            .unwrap();
        assert_eq!(version, 16, "After all migrations, MAX(version) must be 16 (contiguous v1..v16; v17/v18 reports, v19 exam, v20 entitlements all removed)");
    }
    ...
    fn test_apply_migrations_idempotent() {
        ...
        let count: i32 = conn
            ...
            .unwrap();
        assert_eq!(count, 16, "Idempotent: exactly sixteen rows in schema_migrations (contiguous v1..v16; v17/v18 reports, v19 exam, v20 entitlements all removed)");
    }
    ...
    fn test_migration_skip_when_already_applied() {
        ...
        // registered migrations (v3..v16 — the contiguous chain).
        // Max is now 16.
        assert_eq!(version, 16, "current_version returns MAX(version) = 16 after remaining migrations applied (contiguous v1..v16)");
    }
    ...
    // v018 backfill_capability_tags tests (backfills_capability_tags_from_
    // existing_skills_json / backfill_is_idempotent / backfill_skips_rows_
    // already_written_by_production_writer) removed in the Phase 20 reports
    // strip along with the v017/v018 migrations they exercised.
}
