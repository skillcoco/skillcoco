...
use printpdf::{
    BuiltinFont, Color, Mm, Op, PdfDocument, PdfFontHandle, PdfPage, PdfSaveOptions, RawImage,
    Rgb, XObjectTransform,
};
...
///
/// Phase 08.1 (Cert Split) — `qr_png_bytes` is now optional. When `Vec::new()`
/// is passed (OSS unsigned-cert path), the QR section is skipped entirely.
/// Layout below the QR slot stays the same so the brand mark + fingerprint
/// footer keep their pinned positions. Studio passes a non-empty buffer to
/// embed the QR image. The footer line that exposes the key fingerprint
/// is also suppressed when no QR is present (no signed payload = nothing
/// to verify against).
...
///   - "SKILLCOCO" brand mark       Helvetica-Bold 10pt y=30
...
pub fn render_certificate_pdf(
    ...
    let mut doc = PdfDocument::new("SkillCoco Certificate of Completion");

    // Phase 08.1 (Cert Split) — only embed the QR XObject when caller
    // supplied bytes. OSS unsigned-cert path passes Vec::new() and skips
    // the QR + fingerprint footer; Studio overlay path passes a rendered
    // QR PNG and the embed runs as before.
    let qr_xobject_id = if input.qr_png_bytes.is_empty() {
        None
    } else {
    ...
    push_line(&mut ops, &font_bold, 10.0, 20.0, 30.0, "SKILLCOCO");

    // Fingerprint footer — only when a QR/signature is present (Studio).
    // OSS unsigned certs skip this line because there is no signature
    // to verify against (the cert payload still exists, just not signed).
    if qr_xobject_id.is_some() {
        push_line(
            &mut ops,
            &font_reg,
            8.0,
        ...
        // OSS unsigned PDFs get a forward-compatible tagline pointing
        // at the future hosted verifier (Phase 14). The URL is intentionally
        // a placeholder for now — when Phase 14 lands a real verifier, the
        // tagline becomes a live link without changing the cert payload.
        push_line(
            &mut ops,
            &font_reg,
            8.0,
    ...
    // entirely when caller passed an empty QR buffer (OSS unsigned path).
    if let Some(qr_id) = qr_xobject_id {
...
// `push_line` moved to `crate::pdf_util` (Phase 18 Plan 04) — shared with
// the report renderer so the printpdf Td-relative fix cannot be
// re-implemented (and potentially re-broken) a second time.
...
mod tests {
    ...
    fn sample_badge_input() -> BadgePngInput {
        BadgePngInput {
            ...
            issued_at: "2026-06-15T00:00:00Z".to_string(),
            key_fingerprint_short: "deadbeef".to_string(),
            qr_payload: "skillcoco-test-payload-aGVsbG8.deadbeef".to_string(),
        }
    }

    #[test]
    fn pdf_smoke() {
        let qr_png = render_qr_png("skillcoco-test-qr-payload").expect("qr png");
    ...
    fn pdf_length_grows_with_content() {
        ...
        let qr_png = render_qr_png("skillcoco-test-payload").expect("qr png");
        let input = sample_pdf_input(qr_png);
        let bytes = render_certificate_pdf(&input).expect("pdf bytes");
        assert!(bytes.len() >= 2048, "expected >= 2KB, got {}", bytes.len());
    }

    // `push_line_emits_self_contained_text_object` regression test moved to
    // `crate::pdf_util::tests` (Phase 18 Plan 04) alongside the extracted
    // `push_line` helper it guards.

    /// Phase 08.1 (Cert Split) — when the caller passes an empty
    /// `qr_png_bytes` buffer, the PDF renders successfully without QR
    /// embedding. This is the OSS unsigned-cert path. The brand mark +
    /// "Generated by SkillCoco" tagline still appear; the
    /// "Verify with key fingerprint" footer is suppressed.
    #[test]
    fn pdf_renders_without_qr_when_bytes_empty() {
        let input = sample_pdf_input(Vec::new());
        let bytes = render_certificate_pdf(&input).expect("pdf bytes");
        assert!(bytes.starts_with(b"%PDF-"));
        assert!(bytes.len() >= 1024, "PDF should be >= 1KB, got {}", bytes.len());
        // The PDF should NOT carry the Verify-with-fingerprint line because
        // there's no signature to verify against. printpdf emits text
        // content streams; the line bytes survive into the serialized
        // output (potentially within a content stream we can't easily
        // grep, so we only assert positively here: it renders & opens).
    }

    /// Phase 08.1 — Studio path: when QR bytes are non-empty, the embed
    /// happens (no regression vs. pre-split behavior).
    #[test]
    ...
    fn qr_dep_resolution_smoke() {
        ...
        let code = QrCode::new("skillcoco-wave-2-smoke").expect("qrcode 0.14 builds");
    ...
    #[test]
    fn qr_png_smoke() {
        let bytes = render_qr_png("skillcoco-test-payload").expect("qr png bytes");
